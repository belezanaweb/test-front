...
export function CartDetailsProvider({ children }: CartDetailsProviderProps) {
  const [cartDetails, setCartDetails] = useState < CartDetails > cartDetailsInitialValues
  ...
  const getCartDetails = async () => {
    try {
      api.get <
        CartDetails >
        'http://www.mocky.io/v2/5b15c4923100004a006f3c07'.then((cart) => {
  ...
  return <CartDetailsContext.Provider value={cartDetails}>{children}</CartDetailsContext.Provider>
}
