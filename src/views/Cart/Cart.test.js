...
describe('Cart', () => {
  it('should render [snapshot]', () => {
    const { container } = render(<Cart location={mockLocation} />)
  ...
  describe('clicking button', () => {
    it('should call navigate to /checkout/payment passing state as-is', () => {
      const { getByTestId } = render(<Cart location={mockLocation} />)
      expect(navigate).not.toHaveBeenCalled()
      fireEvent.click(getByTestId('button-next'))
      expect(navigate).toHaveBeenCalledWith('/checkout/payment', { state: mockState })
    })
  })
})
