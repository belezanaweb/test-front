...
import * as maskCardNumberExports from '~/utils/mask-card-number';
import { formatNumberToString } from '~/utils/yup-transform-date';
...
const fieldValues = ['2222.2222.2222.2222', 'HUGO MARCELO', '12/2021', '444'];
...
describe('PaymentFormContainer Component', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('FormInput Component', () => {
    ...
    describe('CardHolderName Input', () => {
      it('should show an invalid message when the name is invalid', async () => {
        ...
        await forceErrorMessage(wrapper, 'card_holder_name', 'HUGO ');
        expect(wrapper.find('form span').text()).toEqual('Escreva um nome válido.');

        await forceErrorMessage(wrapper, 'card_holder_name', 'HUGO 123231321');
        expect(wrapper.find('form span').text()).toEqual('Escreva um nome válido.');
      });
      ...
      it('cannot should show an invalid message when the name is valid', async () => {
        ...
        await forceErrorMessage(wrapper, 'card_holder_name', 'HUGO MARCELO');
        expect(wrapper.find('form span')).not.toHaveLength(1);
      });
    });

    describe('CardExpirationDate Input', () => {
